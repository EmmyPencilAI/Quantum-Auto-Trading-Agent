...
const ROUTERS: Record<string, string> = {
  '0x38': '0x10ED43C718714eb63d5aA57B78B54704E256024E', 
  '0x61': '0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3', 
};
...
export const TOKEN_MAP: Record<string, Record<string, string>> = {
  ...
  '0x61': {
    ...
    SOL: '0x18Bbf2202611e967A6679D6D3A13661139E99039', 
    XRP: '0x1D2F0DA4364115C9c7352D45b3E2138f126BCEE1', 
    SUI: '0x18Bbf2202611e967A6679D6D3A13661139E99039'  
  }
};
...
export async function executeRealSwap(
  ...
  if (!fromAddress || !toAddress) throw new Error("Token mapping missing");
  ...
  try {
    if (fromSymbol === 'BNB') {
      return await router.swapExactETHForTokens(amountOutMin, path, userAddress, deadline, { value: amountIn });
    } else {
      const tokenContract = new ethers.Contract(fromAddress, ERC20_ABI, signer);
      ...
      if (toSymbol === 'BNB') {
        return await router.swapExactTokensForETH(amountIn, amountOutMin, path, userAddress, deadline);
      } else {
        return await router.swapExactTokensForTokens(amountIn, amountOutMin, path, userAddress, deadline);
      }
    }
  } catch (err: any) {
    if (err.code === 'ACTION_REJECTED') throw new Error("Transaction rejected.");
    if (err.message?.includes("INSUFFICIENT_OUTPUT_AMOUNT")) throw new Error("Price slipped beyond 1% tolerance.");
    throw err;
  }
}