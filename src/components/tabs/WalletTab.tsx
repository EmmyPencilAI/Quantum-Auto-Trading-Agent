...
export default function WalletTab({ user, mode }: WalletTabProps) {
  ...
  const balances = [
    { symbol: 'BNB', name: 'Binance Coin', balance: '1.452', value: '$842.12', icon: 'https://cryptologos.cc/logos/binance-coin-bnb-logo.png' },
    { symbol: 'USDT', name: 'Tether', balance: '1,250.00', value: '$1,250.00', icon: 'https://cryptologos.cc/logos/tether-usdt-logo.png' },
    { symbol: 'USDC', name: 'USD Coin', balance: '450.00', value: '$450.00', icon: 'https://cryptologos.cc/logos/usd-coin-usdc-logo.png' },
  ];
  ...
  return (
    <div className="space-y-6">
      {/* Wallet Header Card */}
      <div className="relative p-8 rounded-[2rem] bg-gradient-to-br from-blue-600 to-blue-800 overflow-hidden shadow-[0_20px_50px_rgba(37,99,235,0.3)]">
        ...
        <div className="relative z-10">
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-3">
              ...
              <span className="font-bold text-lg tracking-tight">MAIN WALLET</span>
            </div>
          ...
          <div className="mb-8">
            ...
            <h2 className="text-4xl md:text-5xl font-black tracking-tighter">$2,542.12</h2>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2 px-4 py-2 bg-black/20 backdrop-blur-md rounded-xl border border-white/10 font-mono text-xs">
              ...
              <button onClick={handleCopy} className="hover:text-blue-300 transition-colors">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { icon: Plus, label: 'Fund Trading', color: 'bg-blue-500' },
          { icon: Send, label: 'Send', color: 'bg-purple-500' },
          { icon: ArrowDownLeft, label: 'Receive', color: 'bg-green-500' },
          { icon: History, label: 'History', color: 'bg-orange-500' },
        ].map((action, i) => (
          <button 
            key={i}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between px-2">
            <h3 className="font-bold text-lg uppercase tracking-widest text-white/50">Your Assets</h3>
            <button className="text-blue-500 text-xs font-bold hover:underline">View All</button>
          </div>
          <div className="space-y-2">
            {balances.map((asset, i) => (
              <div key={i} className="p-4 rounded-2xl bg-white/[0.03] border border-white/5 flex items-center justify-between hover:bg-white/[0.05] transition-all">
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs text-white/40">{asset.value}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between px-2">
            <h3 className="font-bold text-lg uppercase tracking-widest text-white/50">Recent Activity</h3>
            <button className="text-blue-500 text-xs font-bold hover:underline">History</button>
          </div>
          <div className="space-y-2">
              ...
              <div key={i} className="p-4 rounded-2xl bg-white/[0.03] border border-white/5 flex items-center justify-between hover:bg-white/[0.05] transition-all">
                <div className="flex items-center gap-4">
                  <div className={cn(
                    ...
                    tx.type === 'sent' ? "bg-red-500/10 text-red-500" : "bg-blue-500/10 text-blue-500"
                  )}>
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-white/40 font-mono">{tx.from || tx.to}</p>
                  </div>
                </div>
                <div className="text-right">
                  ...
                  <p className="text-[10px] text-white/40 uppercase tracking-widest">{tx.time}</p>
                </div>
              </div>
            ))}
          </div>
